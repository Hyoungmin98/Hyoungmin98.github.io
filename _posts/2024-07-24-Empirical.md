...
이 논문을 읽고 난 이후에 머신러닝이 이렇게 많이 발전했나?? 이정도 성과면 충분히 제도권에서도 사용할 수 있지 않을까 했지만 $R^2값이 %로 되어있는 것을 뒤늦게 확인했다. 즉 NN3의 $R^2$값이 0.4가 아니라 0.004이었던 것. 1%도 되지않는 모델 설명력으로 실제 투자를 진행하기에는 아직 한계점이 뚜렷한 것으로 보인다.
