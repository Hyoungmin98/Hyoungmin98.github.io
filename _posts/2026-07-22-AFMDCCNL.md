...
이때 조건부 분산의 특징을 살펴보면,  
$d_{i,t}^2 = \operatorname{Var}\left(x_{i,t} \mid \mathcal{F}_{t-1}\right)$   
...
이후 잔차의 조건부 분산을 $d_{i,t}^2 = \operatorname{Var}(x_{i,t} \mid \mathcal{F}_{t-1})$ 라고 정의하면, $\begin{aligned}
...
$\operatorname{Var}(s_{i,t} \mid \mathcal{F}_{t-1}) = \operatorname{Var}\left(\frac{x_{i,t}}{d_{i,t}} \mid \mathcal{F}_{t-1}\right) = 1$  

따라서 평균이 0, 분산이 1이기 때문에 $\begin{aligned}
\mathbb{E}[s_{i,t}^2 \mid \mathcal{F}_{t-1}] &= \operatorname{Var}(s_{i,t} \mid \mathcal{F}_{t-1}) \\
...
$\mathbb{E}[D_{h+l} \mid \mathcal{F}_{h-1}] = \operatorname{Diag}\left(\sqrt{\mathbb{E}[d_{1,h+l}^2 \mid \mathcal{F}_{h-1}]}, \ldots, \sqrt{\mathbb{E}[d_{N,h+l}^2 \mid \mathcal{F}_{h-1}]}\right).$  
...
$\bar{\sum}_b := \frac{1}{L} \sum_{l=0}^{L-1} \hat{\sum}_{h+l}$   
