...
동적 팩터모델 구조에서는 $$\widehat{\Sigma}_{f,t}$$, $$\hat \Sigma_{u,t}$$ 가 필요하기 때문에, 본 논문에서는 다변량 GARCH 모델, 특히 Engle(2002)의 Dynamic conditional correlation(DCC) 를 사용한다.  

Engle(2002)는 N이 100 정도일 때까지는 DCC가 잘 작동하며, $$f_t$$ 의 차원이 거의 5를 넘지 않기 때문에 $$\widehat{\Sigma}_{f,t}$$ 추정에도 사용할 수 있다.  
→ 문제는 자산 수가 많아졌을 때. N = 1000 과 같은 상황에서 $$\Sigma_{u,t}$$ 를 추정하는 것은 어려움이 있음.  
...
이후 $$\mathbb{E}[R_h \mid \mathcal{F}_{h-1}] \approx \mathbb{E}[Q_h \mid \mathcal{F}_{h-1}]$$ 로 근사하면 조건부 상관행렬 $$R_t$$ 또한 정규화 작업을 수행한다.  
...
$$
\mathrm{PL}=\frac{1}{480N}\sum_{h=1}^{480}\sum_{i=1}^{N}\mathbf{1}\!\left\{\widehat{w}_{i,h}<0\right\}
$$
