...
$Q_t = (1 −α−β)C+ αs_{t−1}s_{t−1}' +βQ_{t−1}$  
- $(\alpha, \beta)$: DCC 파라미터  
...
모집단 공분산 행렬을 $\sum$ , 샘플 공분산 행렬을 $S$ , 이 $S$ 의 고유벡터를 $u$ 라고 하면, 표본 고유값은 $u'Su$ 와 같다.  
그리고 이 고유값은 고유벡터 $u$를 가중치로 가지는 포트폴리오의 표본 분산을 의미한다.  
→ NLS(Nonlinear Shrinkage)는 이를 모집단 공분산 행렬을 활용한 $u'\sum u$ 로 대체한다.  
...
$\hat C := \frac{1}{T}SS'$  
이때 행렬 C는 대칭행렬이므로 $u_i'u_j=1 (where \quad i=j)$ 을 만족하고, 고유방정식 $Cu_i = \lambda_iu_i$ 를 따르므로 다음과 같이 나타낼 수 있다.   
$\hat C = \sum_{i=1}^N \lambda_iu_iu_i'$  

$Q_{N,T}$ 를 QuEST 함수라고 하자. 이 함수는 모집단 고유값 집합 $t := (t_1,...,t_N)$ 을 입력으로 주면 표본 고유값 $Q_{N,T}(t) = (q_{N,T}^1(t), ..., q_{N,T}^2(t))$ 를 출력으로 반환한다.  

그러나 우리는 표본 고유값을 알고 모집단 고유값을 얻어야 하므로 QuEST 함수를 수치적으로 역산하여 모집단 고유값을 추정한다.  
$\widetilde{\boldsymbol{\tau}} := \operatorname*{argmin}_{\mathbf{t} \in [0,\infty)^N} \frac{1}{N} \sum_{i=1}^{N} \left[q_{N,T}^i(\mathbf{t}) - \lambda_i\right]^2$  

이렇게 모집단 고유값이 추정되면 NLS를 통해 수축된 고유값 $\tilde \lambda$ 을 계산할 수 있다.  
이후 공분산 행렬의 shrinkage estimator는 다음과 같이 구성된다.  
$\widetilde{C} := \sum_{i=1}^{N} \widetilde{\lambda}_i(\widetilde{\boldsymbol{\tau}}) u_i u_i'$  
...
$\bar λ := \frac{1}{N}\sum_{i=1}^N λ_i$  

이 방법은 샘플 고유값이 횡단면 평균 $\bar λ$ 쪽으로 얼마나 끌릴지를 $ρ$ 를 사용하여 조절한다.  
$\overline{C} := \sum_{i=1}^{N} \left[\rho\,\overline{\lambda} + (1-\rho)\lambda_i\right] u_i u_i'$  
...
$\widehat{C}_{\mathrm{corr}} = \operatorname{Diag}(\widehat{C})^{-1/2} \widehat{C} \operatorname{Diag}(\widehat{C})^{-1/2},$  
$\widetilde{C}_{\mathrm{corr}} = \operatorname{Diag}(\widetilde{C})^{-1/2} \widetilde{C} \operatorname{Diag}(\widetilde{C})^{-1/2},$  
$\overline{C}_{\mathrm{corr}} = \operatorname{Diag}(\overline{C})^{-1/2} \overline{C} \operatorname{Diag}(\overline{C})^{-1/2}.$  
